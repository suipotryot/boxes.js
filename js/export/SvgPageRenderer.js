...
/**
 * @param {{items: {piece:object,x:number,y:number}[], pageWidthMm:number, pageHeightMm:number, label:string}} args
 * @returns {SVGSVGElement}
 */
export function renderSvgPage({ items, pageWidthMm, pageHeightMm, label }) {
  ...
  const pieceGroups = items.map(({ piece, x, y }) => {
    ...
    return svgEl('g', { transform: `translate(${x - bounds.minX} ${y - bounds.minY})` }, [path]);
  });
